        ...
        <v-layout>
            <v-flex xs12>
                <panel :title="$t('reference_source_type')">
                    ...
                    <v-card-actions>
                        <v-spacer></v-spacer>
                        <v-btn color="indigo" dark @click="navigateTo({name: 'reference-source-type.index'})">
                            <v-icon dark left>arrow_back</v-icon>{{ $t('return')}}
                        </v-btn>
                        <v-btn color="indigo" dark @click="navigateTo({name: 'reference-source-type.edit'})">
        ...
        methods: {
            ...
            async deleteType() {
                ...
                api.delete(href)
                    ...
                    this.displayDeleteError(e, 'error_delete_reference_source_type_text', 'error_delete_reference_source_type_detail',
                        'error_not_found_reference_source_type_text', 'error_not_found_reference_source_type_detail');
